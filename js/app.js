 ...
 */            (function(window, factory) {
    ...
    (() => {
        ...
        const cardWrapper = document.querySelector(".curses-bord__body");
        const navList = document.querySelector(".curses-bord__list-groups");
        function windowload() {
            if (cardWrapper && navList) loadcard();
        }
        async function loadcard() {
            const pathData = "files/curses.json";
            const response = await fetch(pathData, {
                metod: "GET"
            });
            if (response.ok) {
                const resolt = await response.json();
                const ourdata = resolt.curses;
                let groupList = [];
                for (let i = 0; i < ourdata.length; i++) groupList[i] = ourdata[i].group;
                const sortList = clonEater(groupList);
                let limit = 6;
                setnav(groupList, sortList);
                getnavitem(ourdata, limit);
                template(ourdata, limit);
                const showWrapper = document.querySelector(".curses-bord__button-wrapper");
                let templateShow = `<button id="standart" type="button" class="curses-bord__load-more">Load more</button>`;
                showWrapper.insertAdjacentHTML("beforeend", templateShow);
                const showstandart = document.getElementById("standart");
                showstandart.addEventListener("click", addstandart);
                function addstandart(event) {
                    limit += 3;
                    let template;
                    ourdata.forEach((item => {
                        if (ourdata.indexOf(item) < limit && ourdata.indexOf(item) > limit - 4) {
                            template = `\n                    <a id="cid${item.id}" href="#" class="curses-bord__card card">\n                    <div class="card__image">\n                        <img src="${item.photo}" alt="photo">\n                    </div>\n                    <article class="card__info">\n                        <div class="card__group ${item.groupMod}">${item.group}</div>\n                        <h3 class="card__title">${item.title}</h3>\n                        <div class="card__subblock">\n                            <div class="card__prise">${item.prise}</div>\n                            <div class="card__name">${item.name}</div>\n                        </div>\n                    </article>\n                </a>`;
                ...
                input.addEventListener("focus", removePlace);
                input.addEventListener("blur", removePlace);
                let placeholder = input.placeholder;
                function removePlace(event) {
                    if (event.type == "focus") input.placeholder = "";
                    if (event.type == "blur") input.placeholder = placeholder;
                }
            }
        }
        ...
        function setnav(groupList, sortList) {
            let navTemplate = `    <li id="All" class="curses-bord__group"><a href="#"><span>All</span><span>${groupList.length}</span></a></li>`;
            navList.insertAdjacentHTML("beforeend", navTemplate);
            sortList.forEach((itemList => {
                navTemplate = `    <li id="${itemList}" class="curses-bord__group"><a href="#"><span>${itemList}</span><span>${clonCounter(itemList)}</span></a></li>`;
                navList.insertAdjacentHTML("beforeend", navTemplate);
            }));
            function clonCounter(item) {
                let count = 0;
        ...
        function getnavitem(data, limit) {
            const navitem = document.querySelectorAll(".curses-bord__group");
            navitem.forEach((item => {
                const id = item.id;
                const sortBase = sortbord(data, id);
                clicklinck(sortBase, id, limit);
            }));
        }
        function clicklinck(data, id, limit) {
            const category = document.getElementById(id);
            category.addEventListener("click", setSortBord);
            function setSortBord(event) {
                event.preventDefault();
                limit = 6;
                cardWrapper.innerHTML = "";
                template(data, limit);
                const showWrapperClick = document.querySelector(".curses-bord__button-wrapper");
                showWrapperClick.innerHTML = "";
                let templateShow = `<button id="${id}btn" type="button" class="curses-bord__load-more">Load more</button>`;
                showWrapperClick.insertAdjacentHTML("beforeend", templateShow);
                const showMore = document.getElementById(id + "btn");
                showMore.addEventListener("click", addmore);
                function addmore(event) {
                    limit += 3;
                    let template;
                    data.forEach((item => {
                        if (data.indexOf(item) < limit && data.indexOf(item) > limit - 4) {
                            template = `\n                    <a id="cid${item.id}" href="#" class="curses-bord__card card">\n                    <div class="card__image">\n                        <img src="${item.photo}" alt="photo">\n                    </div>\n                    <article class="card__info">\n                        <div class="card__group ${item.groupMod}">${item.group}</div>\n                        <h3 class="card__title">${item.title}</h3>\n                        <div class="card__subblock">\n                            <div class="card__prise">${item.prise}</div>\n                            <div class="card__name">${item.name}</div>\n                        </div>\n                    </article>\n                </a>`;
                            cardWrapper.insertAdjacentHTML("beforeend", template);
                        }
                    }));
        ...
        function template(data, limit) {
            let template;
            data.forEach((item => {
                if (data.indexOf(item) < limit) {
                    template = `\n        <a id="cid${item.id}" href="#" class="curses-bord__card card">\n        <div class="card__image">\n            <img src="${item.photo}" alt="photo">\n        </div>\n        <article class="card__info">\n            <div class="card__group ${item.groupMod}">${item.group}</div>\n            <h3 class="card__title">${item.title}</h3>\n            <div class="card__subblock">\n                <div class="card__prise">${item.prise}</div>\n                <div class="card__name">${item.name}</div>\n            </div>\n        </article>\n    </a>`;
                    cardWrapper.insertAdjacentHTML("beforeend", template);
                }
            }));
        }
        function sortbord(base, elem) {
            let sortBase = [];
            for (let g = 0; g < base.length; g++) {
                if (elem == "All") sortBase[g] = base[g];
                if (base[g].group == elem) sortBase.push(base[g]);
            }
            return sortBase;
        }
        ...
        addClasser(".curses-bord__load-more");
        addClasser(".curses-bord__group");
        window["FLS"] = true;
        isWebp();
    })();
})();